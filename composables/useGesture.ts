...
export const useGesture = (
  ...
  const handleTouchMove = useThrottleFn((e: TouchEvent) => {
    ...
    if (e.touches.length === 1 && touchPoints.value.size === 1) {
      ...
      if (
        ...
        (Math.abs(deltaX.value) > tapThreshold || Math.abs(deltaY.value) > tapThreshold)
      ) {
  ...
  const setup = () => {
    ...
    const removeTouchMove = useEventListener(
      element,
      'touchmove',
    ...
    const removeTouchCancel = useEventListener(
      element,
      'touchcancel',
      handleTouchCancel,
      touchOptions
  ...
  return {
    ...
    // Methods
    on: <T extends GestureType>(type: T, callback: (event: GestureEvent) => void) => {
      const callbackMap = {
        swipe: 'onSwipe',
        pinch: 'onPinch',
        rotate: 'onRotate',
