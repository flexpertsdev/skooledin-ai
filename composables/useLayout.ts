...
export const useLayout = () => {
  ...
  const contentOffset = computed(() => {
    const top = navbarVisible.value && layoutState.navbar.fixed ? layoutState.navbar.height : 0

    const bottom = bottomNavVisible.value ? layoutState.bottomNav.height : 0
  ...
  const contentMargin = computed(() => {
    const left =
      ...
      !layoutState.sidebar.overlay &&
      layoutState.sidebar.position === 'left'
    ...
    const right =
      sidebarVisible.value &&
      !layoutState.sidebar.overlay &&
      layoutState.sidebar.position === 'right'
        ? layoutState.sidebar.mini
          ? 64
          : layoutState.sidebar.width
        : 0
